...
def run(protocol: protocol_api.ProtocolContext,data=None) -> None:
    ...
    HCl_stock = Vial({'H+':9,'Cl-':9},{'H2O':80},tube_rack,0,80,complete=True)
    HBr_stock = Vial({'H+':9,'Br-':9},{'H2O':80},tube_rack,1,80,complete=True)
    HI_stock = Vial({'H+':9,'I-':9},{'H2O':80},tube_rack,2,80,complete=True)
    ...
    for i in range(0,5):
        solutions.append(Vial({'Cs+':(i+1)*powder_mmols,'Ac-':(i+1)*powder_mmols},{},heater_shaker_plate,i,5))
